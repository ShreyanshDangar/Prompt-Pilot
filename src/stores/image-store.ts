...
interface ImageStore {
  ...
  clearImages: () => void
  getTotalImageTokens: (formula: (w: number, h: number) => number) => number
  initialize: () => void
}
...
export const useImageStore = create<ImageStore>((set, get) => ({
  ...
  getTotalImageTokens: (formula) => {
    return get().images.reduce(
      (total, img) => total + formula(img.width, img.height),
      0
    )
  },
